...
ip_addresses = ['http://75.119.155.29/', 'https://utils.blocknet.org/', 'http://149.102.153.235/']
# servers: conan, blocknet (conan), shorn
...
def fake_data():
    ...
    my_dict = {
        ...
        'url_node3': {
            ...
            'DASH': (1813785, '000000000000002dadab73fb1880a606a6105156eed3451619abf820c5e34a0e'),
            'LTC': (2413782, 'd89ee7a66506f97ceca7a40b2b853ac2d7260f78cff754467219d252e89b5a65'),
            'PIVX': (3726151, 'sameheight_differenthash'),
            'SYS': (1556147, '481eca73fe77bd5c93a795f39d9ee6161bf7fd90d854649a992527070b5ca9e1')},
...
while 1:
    ...
    for key in all_keys:
        values = [(blockchain_lengths[ip][key][0], blockchain_lengths[ip][key][1]) if key in blockchain_lengths[ip] else (None, None) for ip in ip_addresses
                if
                blockchain_lengths[ip] is not None]
        # Determine the majority value for this key
        print('key', key, 'values:', values)
        counter_obj = Counter(values)
        most_common = counter_obj.most_common()[0]
        # print("most_common:", most_common[0], most_common[1])
        if most_common[1] >= min_nodes_consensus and snodes_count >= 3:
            if all(value == most_common[0] for value in values if value != (None, None)):
                resolved_data[key] = most_common[0]
            else:
